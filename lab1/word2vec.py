...
def naiveSoftmaxLossAndGradient(
    ...
    probability = softmax(np.matmul(outsideVectors, centerWordVec))
    loss = -np.log(probability[outsideWordIdx])
    a = np.zeros_like(outsideVectors[0])
    for i, p in enumerate(probability):
        a += p * outsideVectors[i]
    gradCenterVec = a - outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.zeros_like(outsideVectors)
    for i, gradOutsideVec in enumerate(gradOutsideVecs):
        gradOutsideVecs[i] = probability[i] * centerWordVec
        if i == outsideWordIdx:
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentWordVector = centerWordVectors[currentWordIndex]
    for word in outsideWords:
        loss_, gradCenterVec_, gradOutsideVector_ = word2vecLossAndGradient(currentWordVector,
                                                                            word2Ind[word],
                                                                            outsideVectors, dataset)
        loss += loss_
        gradCenterVecs[currentWordIndex] += gradCenterVec_
...
if __name__ == "__main__":
    ...
    # a = softmax(np.matmul(outsideVectors, centerWordVec))
    # print(a)
