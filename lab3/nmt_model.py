...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens = pad_packed_sequence(enc_hiddens, padding_value=self.vocab.src["<pad>"])[0].permute(1, 0, 2)
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=-1))
        init_decoder_cell = self.h_projection(torch.cat((last_cell[0], last_hidden[1]), dim=-1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # to [b,len,h]
        Y = self.model_embeddings.target(target_padded)  # Y [tgt_len,b,e]
        for chunk in torch.split(Y, 1, dim=0):
            dec_state, o_prev, e_t = self.step(torch.cat((chunk.squeeze(0), o_prev), dim=-1), dec_state, enc_hiddens,
                                               enc_hiddens_proj,
                                               enc_masks)
