...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            word2 = self.stack.pop(len(self.stack) - 2)
            self.dependencies.append((self.stack[-1], word2))
        elif transition == 'RA':
            word1 = self.stack.pop(len(self.stack) - 1)
            self.dependencies.append((self.stack[-1], word1))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        part_parser = unfinished_parses[:batch_size]
        transition_list = model.predict(part_parser)
        for i in range(len(part_parser)):
            parser = part_parser[i]
